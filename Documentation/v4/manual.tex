...
Peakaboo is a part of the Science Studio package that allows users to identify the 
spectral origins of an XRF spectrum using a routine that fits all components of 
the K, L, or M spectrum including escape peaks and pileup peaks, and then plots 
their spatial intensity distributions as maps.

\tocsection{Accepted File Formats}

Peakaboo supports several file formats for XRF data, including: 

\begin{enumerate}

\item \command{CDFML}, an xml-based file format defined by the Canadian Light 
Source using NASA's CDF/CDFML container.

\item \command{Peakaboo Plaintext Format}, a simple XRF format comprised of 
rows of space-separated numbers. Each row represents a single spectrum.

\item \command{CLS Data Acquisition Format} used by the Canadian Light Source 
for XRF collection on the VESPERS beamline.

\end{enumerate}

For a complete listing of supported file formats, see ``File Format Descriptions''
in the appendix.
...
to contact the developers at \command{nsherry4@uwo.ca}
...
containing Peakaboo for building against, as part of the Peakaboo 4 Software Development Kit.
