...
containing Peakaboo for building against, as part of the Peakaboo 5 Software Development Kit.
...
For more complex \class{Parameter} value types, a parameter may have properties associated with it. 
...
The \class{Spectrum} class has a \class{List}-like interface, but uses a fixed-size \class{float} array to 
store the actual data for memory and performance reasons.  If you prefer to work with the \class{float[]} data directly, the \code{backingArray}
...
\code{super}, the superclass's constructor. If you are attempting to integrate existing code and cannot extend 
\class{AbstractDSP}, the \class{DataSource} interface (which \class{AbstractDSP} implements) can be implemented 
instead, and wrapped in a \class{DelegatingDSP}.


\tocsubsection{Defining File Extensions}

Providing a list of file extensions that your data will typically have allows users to browse for your data format
...
reading in the whole data set just to see if it can, it should still perform enough of a check to differentiate it 
...
reports that a certain number of new scans have been read. These two methods used together allow Peakaboo to
