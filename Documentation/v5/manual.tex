...
Peakaboo is a platform that facilitates the interpretation of spectra from X ray Fluorescence (EXF) experiments that use conventional or synchrotron-based radiation sources. It uses efficient software tools to provide a close fitting of most spectral components using the most recent libraries available. A prime objective of the platform is to provide detailed maps of elemental spatial distribution(s) and attendant tools to extract compositional information from each region of a map. The simplified software layout makes it possible for new users to be productive within a matter of minutes.

Peakaboo provides only qualitative information about elemental composition. Potential users interested in more advanced versions of the software should contact the developers.

Peakaboo was originally developed within the Science Studio project that was funded by CANARIE.
...
\tocsection{Saving and Loading Data}

From the toolbar, you can \button{document-save-as}{Save} your session information so that next time you want to look at this XRF dataset, you can load this information (session) and not have to start from the beginning. The saved session does not store the data itself, so the data must still be available when loading a session.
...
After you have added the filter, you can modify or check the parameters associated with your filter by clicking on the \button{misc-preferences}{Edit} button next to the filter. This will bring up a settings panel for this filter. As you change the parameters, you will see the changes reflected in the spectrum being displayed.
...
While the removal of background certainly makes the actual spectral peaks more apparent, we have found that when fitting peaks to a spectrum, it is often not necessary to carry out a background subtract. The peaks fitted using a Voigt function (pseudo or convolved) actually contain a significant component of the scattering function that approximates the background. This is particularly true in the energy range 1-9 KeV. Therefore we recommend that spectral fitting be first carried out without any background subtraction

For a more detailed example of using a filter, see the appendix.
...
\screenshot{fitting-markings}{Spectral Fitting of multi element NIST 610 Glass showing individual lines}
...
Point selection is accomplished by double clicking on a point of interest, which will select all points in a contiguous region with similar intensity. Triple clicking will select all points in a map with similar intensity regardless of location. More complex selections can be made by holding the \command{Control} key while double-clicking. For example, \command{Control} + \command{Double Clicking} on a previously unselected point will create a union with a previously selected set of points.

The \command{Map Settings} tab has two settings which will control how point selections are made. 
