...
public class StratusFilePanels {
	
	private static void showChooser(Component parent, StratusFileChooser chooser, Runnable onAccept, Runnable onCancel, String title) {
		if (LayerPanel.parentOf(parent)) {
			...
			chooserPanel.setPreferredSize(new Dimension(950, 450));
	...
	private static StratusFileChooser getOpener(String title, File startingFolder, List<SimpleFileExtension> extensions) {
		StratusFileChooser chooser = new StratusFileChooser(startingFolder);
		chooser.setDialogTitle(title);
	...
	{
		StratusFileChooser chooser = getOpener(title, startingFolder, extensions);
		chooser.setMultiSelectionEnabled(false);
