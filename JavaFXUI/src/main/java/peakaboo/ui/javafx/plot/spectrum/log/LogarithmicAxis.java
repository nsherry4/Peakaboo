...
public class LogarithmicAxis extends ValueAxis<Number> {
	...
	private static double zerolog(double value) {
		// if (value == 0) { return 0; }
		return Math.log10(1 + value);
	}

	private static double zeropow(double base, double value) {
		if (value == 0) { return 0; }
	...
	private void bindLogBoundsToDefaultBounds() {
		logLowerBound.bind(new DoubleBinding() {
			...
			protected double computeValue() {
				return zerolog(lowerBoundProperty().get());
			}
		});
		logUpperBound.bind(new DoubleBinding() {
			...
			@Override
			protected double computeValue() {
				return zerolog(upperBoundProperty().get());
			}
		});
	}
	...
	protected List<Number> calculateTickValues(double length, Object range) {
		...
		if (range != null) {
			...
			double logLowerBound = zerolog(lowerBound.doubleValue());
			double logUpperBound = zerolog(upperBound.doubleValue());
	...
	public Number getValueForDisplay(double displayPosition) {
		...
		if (getSide().isVertical()) {
			return zeropow(10, (((displayPosition - getHeight()) / -getHeight()) * delta) + logLowerBound.get());
		} else {
			return zeropow(10, (((displayPosition / getWidth()) * delta) + logLowerBound.get()));
		}
	}
	...
	public double getDisplayPosition(Number value) {
		double delta = logUpperBound.get() - logLowerBound.get();
		double deltaV = zerolog(value.doubleValue()) - logLowerBound.get();
		if (getSide().isVertical()) {
